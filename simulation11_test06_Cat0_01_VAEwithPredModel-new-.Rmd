...
print(tt)
print((tt[1,1]+tt[2,2])/(sum(tt))*100)
print(tt[1,1]/(tt[1,1]+tt[1,2])*100)
print(tt[2,2]/(tt[2,1]+tt[2,2])*100)
 } 
...
```{r}
temp4 <- predict(model1, as.matrix(dfTS[,-1]))
confusionMatrix(temp4>=0.5, dfTS[,1]) -> tt4
tt4
```

```{r}
(tt4[1,1]+tt4[2,2])/(sum(tt4))*100
```

```{r}
tt4[1,1]/(tt4[1,1]+tt4[1,2])*100
```

```{r}
tt4[2,2]/(tt4[2,1]+tt4[2,2])*100
```
...
# vae 예측
temp4 <- predict(vae, as.matrix(dfTS))

# 앙상블 모델 예측 결과 생성
ensemble_averaging <- (temp3 + temp4) / 2
```

```{r}
# 앙상블 모델의 정확도 평가
confusionMatrix(ensemble_averaging >= 0.5, dfTS[,1])->ttaveraging
ttaveraging

(ttaveraging[1,1]+ttaveraging[2,2])/sum(ttaveraging)*100
```

```{r}
ttaveraging[1,1]/(ttaveraging[1,1]+ttaveraging[1,2])*100
```

```{r}
ttaveraging[2,2]/(ttaveraging[2,1]+ttaveraging[2,2])*100
```
...
# 예측 결과 결합
ensemble_weighted <- (temp3 * weight1) + (temp4 * weight2)

# 앙상블 모델의 정확도 평가
confusionMatrix(ensemble_weighted >= 0.5, dfTS[,1])->ttweighted
ttweighted
```

```{r}
(ttweighted[1,1]+ttweighted[2,2])/sum(ttweighted)*100
```

```{r}
ttweighted[1,1]/(ttweighted[1,1]+ttweighted[1,2])*100
```

```{r}
ttweighted[2,2]/(ttweighted[2,1]+ttweighted[2,2])*100
```

```{r}
## weighted averaging method2 ##
...
another_data <- data.frame(temp3, temp4)


another_data$target <- ifelse(temp3 >= 0.5 & temp4 >= 0.5, 1,
                              ifelse(temp3 >= 0.5 & temp4 < 0.5 & temp3 > (1 - temp4), 1,
                                     ifelse(temp3 >= 0.5 & temp4 < 0.5 & temp3 <= (1 - temp4), 0,
                                            ifelse(temp3 < 0.5 & temp4 >= 0.5 & (1 - temp3) > temp4, 0,
                                                   ifelse(temp3 < 0.5 & temp4 >= 0.5 & (1 - temp3) <= temp4, 1,
                                                          ifelse(temp3 < 0.5 & temp4 < 0.5, 0, 1))))))


# 예측 결과를 데이터 프레임으로 변환
another_data <- data.frame(temp3, temp4, target = another_data$target)

confusionMatrix(as.matrix(another_data$target), dfTS[,1])->ttanother
ttanother
```

```{r}
(ttanother[1,1]+ttanother[2,2])/sum(ttanother)*100
```

```{r}
ttanother[1,1]/(ttanother[1,1]+ttanother[1,2])*100
```

```{r}
ttanother[2,2]/(ttanother[2,1]+ttanother[2,2])*100
```